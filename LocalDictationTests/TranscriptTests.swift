...
final class LanguageProfileTests: XCTestCase {
    ...
    func testProfileLookupRoundTripsThroughItsIdentifier() {
        for profile in LanguageProfile.all {
            XCTAssertEqual(LanguageProfile.profile(id: profile.id), profile)
        }
        XCTAssertNil(LanguageProfile.profile(id: "pl"))
    }

    func testMixedProfileExposesBothLanguages() {
        XCTAssertEqual(LanguageProfile.germanEnglish.languages, [.german, .english])
        XCTAssertEqual(LanguageProfile.germanEnglish.shortLabel, "DE+EN")
        XCTAssertTrue(LanguageProfile.germanEnglish.isMixed)
        XCTAssertTrue(LanguageProfile.germanEnglish.contains(.english))
        XCTAssertFalse(LanguageProfile.germanEnglish.contains(.russian))
    }

    func testDefaultProfileTargetsTheInitialMarket() {
