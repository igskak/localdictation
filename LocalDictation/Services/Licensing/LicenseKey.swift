...
struct LicenseAuthority: Sendable {
    /// Base64 (standard, not URL) of the 32-byte Ed25519 public key. Empty in a
    /// development build, which is why one cannot accept any license at all.
    ///
    /// Run `swift Tools/licensekit.swift init` to generate a keypair; it prints
    /// the line to paste here and writes the private half outside the
    /// repository.
