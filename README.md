...
4. Build and run. The first launch does the asking: it opens a window, and that window asks macOS for the microphone and for Accessibility while explaining what each is for.
5. The first launch asks which languages you speak. Pick as many as you use — there is nothing to switch between afterwards, and **Settings → Languages** changes it later. Behind that window the app is already downloading roughly 600 MB of Whisper weights into `~/Library/Application Support/Witness/Models`, because nothing in the product works until they are here. This is the only network access in the app and it is a one-way fetch of a static asset. It shows its progress in the menu bar and on the first-run screen, and a press of `⌥Space` during it is answered with how long is left rather than with silence.
...
7. Accessibility is asked for on the first run, and again by the first insertion if it was not granted then. Until it is granted, results are copied to the clipboard instead. **A development build loses this permission on every rebuild**, because macOS keys the grant to the code signature — expect to re-grant it after each build from Xcode.
8. The menu carries one per-session choice: *Any of RU+EN+UK*, or *Only Russian* for the hour you spend on one document. It is never written to disk. Under **Settings → General** you can change the shortcut, switch to pressing it twice instead of holding it, and have the app open at login. The microphone choice is there too: Witness prefers the Mac's built-in microphone so dictation does not need to take AirPods away from an iPhone; you can instead choose the macOS default or a particular available microphone. This changes Witness's input only, applies to the next recording, and survives a relaunch. The shortcut needs at least one of `⌘ ⌥ ⌃ ⇧`; a combination something else already owns is refused and the working one stays. Opening at login needs the app to live in `/Applications` — macOS will not make a login item out of a build running from Xcode, and says so.
